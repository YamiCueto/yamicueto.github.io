...
fs.writeFileSync(path.join(output, 'Yamid_Cueto_Fullstack_Engineer_CV.pdf'), result.bytes);
fs.writeFileSync(path.join(output, 'Yamid_Cueto_Amadeus_R35120.pdf'), result.bytes);
fs.writeFileSync(path.join(cvDir, 'Yamid_Cueto_Fullstack_Engineer_CV.pdf'), result.bytes);
fs.writeFileSync(path.join(cvDir, 'Yamid_Cueto_Amadeus_R35120.pdf'), result.bytes);
fs.writeFileSync(path.join(output, 'layout.json'), JSON.stringify(result.layout, null, 2) + '\n');
console.log(`Full Stack CV: ${result.pages} A4 pages, ${result.bytes.length} bytes; repeat generation identical.`);
